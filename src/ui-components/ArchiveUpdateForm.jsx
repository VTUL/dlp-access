...
import { updateArchive } from "../graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function ArchiveUpdateForm(props) {
  const {
    id: idProp,
    archive: archiveModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = archiveRecord
      ? { ...initialValues, ...archiveRecord }
      : initialValues;
    setAlternative(cleanValues.alternative ?? []);
    setCurrentAlternativeValue("");
    setArchiveOptions(
      typeof cleanValues.archiveOptions === "string" ||
        cleanValues.archiveOptions === null
        ? cleanValues.archiveOptions
        : JSON.stringify(cleanValues.archiveOptions)
    );
    setBasis_of_record(cleanValues.basis_of_record ?? []);
    ...
    setConforms_to(cleanValues.conforms_to ?? []);
    setCurrentConforms_toValue("");
    setContributor(cleanValues.contributor ?? []);
    ...
    setHas_format(cleanValues.has_format ?? []);
    setCurrentHas_formatValue("");
    setHas_part(cleanValues.has_part ?? []);
    setCurrentHas_partValue("");
    setHas_version(cleanValues.has_version ?? []);
    ...
    setIs_format_of(cleanValues.is_format_of ?? []);
    setCurrentIs_format_ofValue("");
    setIs_part_of(cleanValues.is_part_of ?? []);
    setCurrentIs_part_ofValue("");
    setIs_version_of(cleanValues.is_version_of ?? []);
    setCurrentIs_version_ofValue("");
    setItem_category(cleanValues.item_category);
    setLanguage(cleanValues.language ?? []);
    setCurrentLanguageValue("");
    setLicense(cleanValues.license ?? []);
    setCurrentLicenseValue("");
    setLocation(cleanValues.location ?? []);
    setCurrentLocationValue("");
    setManifest_file_characterization(
      typeof cleanValues.manifest_file_characterization === "string" ||
        cleanValues.manifest_file_characterization === null
        ? cleanValues.manifest_file_characterization
        : JSON.stringify(cleanValues.manifest_file_characterization)
    );
    setManifest_url(cleanValues.manifest_url);
    setMedium(cleanValues.medium ?? []);
    setCurrentMediumValue("");
    setModified_date(cleanValues.modified_date);
    setOther_identifier(cleanValues.other_identifier ?? []);
    ...
    setRepository(cleanValues.repository ?? []);
    ...
    setCurrentTagsValue("");
    setTemporal(cleanValues.temporal ?? []);
  ...
  const [archiveRecord, setArchiveRecord] = React.useState(archiveModelProp);
  React.useEffect(() => {
    ...
    queryData();
  }, [idProp, archiveModelProp]);
  React.useEffect(resetStateValues, [archiveRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updateArchive.replaceAll("__typename", ""),
            variables: {
              input: {
                id: archiveRecord.id,
                ...modelFields
      ...
      {...getOverrideProps(overrides, "ArchiveUpdateForm")}
      {...rest}
    >
      <ArrayField
      ...
      <TextAreaField
        ...
        isRequired={false}
        isReadOnly={false}
        value={manifest_file_characterization}
        onChange={(e) => {
          let { value } = e.target;
          if (onChange) {
            const modelFields = {
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || archiveModelProp)}
