...
const mediaEmbedText =
  '<iframe src="https://www.youtube.com/embed/8fswmAtvCqI" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';
const titleText = "Welcome to the University Libraries at Virginia Tech";
const textText =
  "The University Libraries play an essential role in furthering Virginia Tech’s mission as a global land-grant university by providing a diversity of resources to produce, disseminate, use, share and sustain data and information.";
...
describe("admin_media_section_config: Displays and updates media section configurations", () => {
  ...
  describe("admin_media_section_config: Updates title and changes it back", () => {
    ...
    it("Reverses update", () => {
      cy.get("input[value='edit']").parent().click();
      cy.get("input[name='title']", { timeout: 2000 }).clear().type(titleText);
      cy.get("button.submit").contains("Update Config").click();
  ...
  describe("admin_media_section_config: Doesn't render media section if no values present", () => {
    it("Clears values and doesn't render section", () => {
      ...
      cy.visit("/");
      cy.reload(true);
      cy.get("div.media-section-wrapper", { timeout: 2000 }).should(
        "not.exist"
      );
      cy.visit("/siteAdmin");
    });
  ...
  describe("admin_media_section_config: Renders media section if values present", () => {
    it("Adds values back and renders section", () => {
      cy.get("input[value='edit']").parent().click();
      cy.get("input[name='link']", { timeout: 2000 }).clear().type(linkText);
      cy.get("textarea[name='mediaEmbed']", { timeout: 2000 })
        .clear()
        .type(mediaEmbedText);
      cy.get("input[name='title']", { timeout: 2000 }).clear().type(titleText);
      cy.get("textarea[name='text']", { timeout: 2000 }).clear().type(textText);
      cy.get("button.submit").contains("Update Config").click();

      cy.visit("/");
      cy.get("div.row.media-section-wrapper", { timeout: 5000 }).should(
        "be.visible"
      );
      cy.visit("/siteAdmin");
    });
