describe("admin_homepage_collectionHighlights_config: Update collection highlights fields and revert", function () {
  ...
  it("Updates first item title", () => {
    ...
    cy.get("#highlight0_title").clear().type("Sketches");
  ...
  it("Reverses update", () => {
    ...
    cy.contains("Update Config").click();
    cy.contains("Title: Travel sketches by IAWA architects").should(
      "be.visible"
    );
  });
  ...
  it("Adds new item with image", () => {
    ...
    cy.get(
      "#collectionHighlight3_form > section > div.fileUploadField > input[type=file]"
    )
      .eq(0)
      .attachFile(imgPath)
    ...
    cy.get("#highlight3_link").type("/search?q=building&view=gallery", {
      force: true
    });
    ...
    cy.contains("highlights/highlight4.jpg").should("be.visible");
    cy.contains("Title: New Highlight").should("be.visible");
    cy.contains("Link: /search?q=building&view=gallery").should("be.visible");
  });

  it("Removes new item", () => {
    cy.get("@homepageLink").click();
    cy.get("input[value='edit']").parent().click();
    cy.get("#collectionHighlight3_form").contains("Remove highlight").click();
    cy.contains("Update Config").click();
